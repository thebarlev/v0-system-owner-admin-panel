-- Migration: Add Receipt Form UI Texts
-- Version: 011
-- Date: 2025-12-29
-- Description: Adds all 70 receipt form UI text keys to system_texts table
-- Prerequisites: Must run 010-system-texts-table.sql first

-- Verify system_texts table exists
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE tablename = 'system_texts') THEN
    RAISE EXCEPTION 'system_texts table does not exist. Run 010-system-texts-table.sql first.';
  END IF;
END $$;
...
-- Document Details Section (6 keys)
INSERT INTO public.system_texts (key, page, default_value, description) VALUES
...
BEGIN
  SELECT COUNT(*) INTO receipt_count FROM public.system_texts WHERE page = 'receipt';
  RAISE NOTICE 'Total receipt form texts: %', receipt_count;
  
  IF receipt_count < 70 THEN
    RAISE WARNING 'Expected at least 70 receipt texts, found only %', receipt_count;
  END IF;
END $$;

-- Create optimized index for receipt page queries
...
BEGIN
  ...
  RAISE NOTICE '✅ Added 70 receipt form UI text keys';
  RAISE NOTICE '✅ Next: Visit /admin/texts to customize values';
END $$;
